Shader "<+AUTHOR+>/UnlitVFFrontBackVFace"
{
    Properties
    {
        ...
        Pass
        {
            ...
            */
            ...
            fixed4 frag(v2f i, fixed facing : VFACE) : SV_Target
            {
                ...
                const float4 fc = mainColor + emissionColor;

                const float4 backColor = UNITY_SAMPLE_TEX2D(_BackTex, i.uv) * _BackColor;
                const float4 backEmissionColor = UNITY_SAMPLE_TEX2D(_BackEmissionTex, i.uv) * _BackEmissionColor;
                const float4 bc = backColor + backEmissionColor;

                float4 col = facing > 0 ? fc : bc;
#ifdef _ENABLEFOG_ON
